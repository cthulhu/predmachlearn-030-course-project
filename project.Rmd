...
predictors = grep("(_belt|_arm|_dumbbell|_forearm)", cols.with.data, value=T)
train.df.clean = cbind(train.df.clean[,c(predictors, "classe")])
```

```{r}
#outcome.org = train.df.clean[, "classe"]
#featurePlot(train.df.clean, outcome.org)
#corrplot.mixed(cor(train.df.clean), lower="circle", upper="color", 
...
random.Forest.fit = train(classe ~ .,
               data = train.final,
               method = "rf",
               trControl = trc)
```
